...
p = configargparse.ArgParser(default_config_files=[os.path.join(current_dir,'netbox-sync.conf')])
p.add('-c', '--config', default=os.path.join(current_dir,'netbox-sync.conf'), is_config_file=True, help="Config file path")
...
p.add('-o', '--get_mac', action='store_true', default=False, help="Enable if you want the script to try to collect MAC addresses/vendor",
      env_var="MAC_DISCOVER")
p.add('-s', '--get_services', action='store_true', default=False, help="Enable if you want the script to discover host services",
...
if __name__ == '__main__':
    ...
    nb = NetBoxHandler(conf.nb_url, conf.nb_token, conf.nb_ignore_tls_errors, conf.tag, conf.clenaup)
    hosts = dict()
